...
```{python}
print("Unique Neuron cluster ID: ", anndata['Neuron_cluster_ID'].unique())
print("Unique Cell Class: ", anndata['Cell_class'].unique())
...
### Downsizing: Filter by neuronal class (Include `Excitatory` / `Inhibitory`; exclude 'Ambiguous')

```{python}
anndata_filtered = anndata[anndata['Cell_class'].isin(['Excitatory','Inhibitory']) &
        (anndata['Cell_class'] != 'Ambiguous')].copy()
anndata_filtered.sample(10)

```

```{python}
print("Unique Cell Class after Filtering: ", anndata_filtered['Cell_class'].unique())

print("Number of Cell Rows after Filtering: ", len(anndata_filtered))
```

### Export Downsized Data (Downsized by `Excitatory` / `Inhibitory` class)

```{python}
anndata_filtered.to_csv('data/proccesed/filtered_neurons.csv', index=False)
