...
```{python}
anndata_filtered = pd.read_csv('data/processed/merfish_sample_100k.csv')
```
...
```{python}
gene_cols = [col for col in anndata_filtered.columns 
...
```{python}
obs = anndata_filtered[metadata_cols]
X = anndata_filtered[gene_cols]
```
...
```{python}
sc.tl.leiden(adata, resolution = .5, random_state=42, n_iterations=2, flavor="igraph")
```
